...
(WIDTH_BUCKET(TRUNC(((to_date('20.12.31') - hire_date)/365)),0,20,30)-1) 보너스등급,
...
--Q. 출판사가 굿스포츠, 혹은 대한미디어인 아닌 도서를 검색하세요.
...
--Q. 고객이 주문한 도서의 총판매액, 평균값, 최저가, 최고가
...
--Q. 고객의 이름과 고객이 주문한 도서의 이름을 구하세요.(3개 조인)
...
SELECT C.NAME, O.SALEPRICE -- 고객 이름, 주문 가격
FROM CUSTOMER C, ORDERS O -- 고객테이블, 주문테이블
WHERE C.CUSTID = O.CUSTID(+); -- 고객테이블에서 고객 ID = 주문테이블에서 고객 ID 조인

--[과제] 가장 비싼 도서의 이름을 출력하세요.(부속 질의, 중첩질의 -> WHERE 절의 부속질의 먼저 처리하고 전체질의를 처리함)
...
update customer
set address = (select address from customer -- set 조건
where name ='김연아')
where name ='박세리';
select * from customer;
...
--Q.Q.마당서점의 고객 중에서 같은 성(姓)을 가진 사람이 몇 명이나 되는지 성별 인원수를 구하시오.
...
select * from orders;
select orderid 주문번호, to_char(orderdate, 'YYYY-mm-dd day') as 주문일, custid as 고객번호, bookid as 도서번호
from orders
where orderdate = '20/07/07';

--Q.고객목록에서 고객번호, 이름, 전화번호를 앞의 두 명만 보이시오.
select * from customer;
select rownum as 순번, custid as 고객번호, name as 이름, phone as 전화번호
from customer
where rownum<=2;
